// ====================================================================
// Void AI Matrix Service (Node.js / Hugging Face Ultimate Edition)
// ====================================================================
...
Registry.set('resolve', (type, buf) => {
    ...
    // 【核心修复1】Node.js net.connect 不接受带中括号的 IPv6
    return ipv6.join(':');
});
...
const renderAIMatrix = (res) => {
    ...
    <head>
        ...
        <title>LLM Node Edge Control</title>
    ...
    <body class="grid-bg min-h-screen p-6 md:p-12">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="glass p-6 rounded-lg flex flex-col">
                    ...
                    <div class="terminal flex-1 text-gray-400" id="terminal">
                        <div class="text-sky-500">>> INITIALIZING AETHER ENGINE v4.2...</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            const logs = [
                ...
                "KV cache synced across distributed nodes.",
                "Attention mechanism optimized: FlashAttention-2 active.",
                "Inference stream #2940 established.",
...
const generateSubscription = (req, res) => {
    ...
    const tag = encodeURIComponent('Aether-Node');
...
// ====================================================================
// Server Initialization & 永动机保活引擎 (Self-Keep-Alive)
// ====================================================================

server.listen(ENV.PORT, () => {
    console.log(`Void AI Matrix running on port ${ENV.PORT}`);
...
function startKeepAlive() {
    // Render 会自动注入 RENDER_EXTERNAL_URL
    // 如果你绑定了自定义域名，也可以手动在环境变量里设置 APP_URL
    const appUrl = process.env.RENDER_EXTERNAL_URL || process.env.APP_URL;

    if (!appUrl) {
        console.log('[Keep-Alive] Warning: 找不到公网 URL，自保活引擎未启动。');
        return;
    }

    // 设置每 8 分钟（480,000 毫秒）执行一次 Ping 操作
    const interval = 8 * 60 * 1000;

    console.log(`[Keep-Alive] 引擎已启动，目标 URL: ${appUrl}`);

    setInterval(() => {
        // 请求我们自己炫酷的 AI 伪装面板根目录
        fetch(appUrl)
            .then(res => {
                if (res.ok) {
                    console.log(`[Keep-Alive] ${new Date().toISOString()} - Ping 成功，状态码: ${res.status}`);
                } else {
                    console.log(`[Keep-Alive] ${new Date().toISOString()} - Ping 异常，状态码: ${res.status}`);
                }
            })
            .catch(err => {
                console.error(`[Keep-Alive] ${new Date().toISOString()} - Ping 失败: ${err.message}`);
            });
    }, interval);
}